df <- read.csv("/Users/hollabharath/Documents/Projects/ADBS/DATA_QC/Scripts/workandsocial_table.csv", header =T)

names(df) <- c("id", "subjectID", "assessmentId", "test_taken_by", 
               "retired", "work", "home", "social", "private", "relationships",
               "deviceid", "date_of_last_update", "total")

# Check for subjects with missing data/non response for exclusion 
df_excluded <- df[is.na(df$total), ]
...
df$WSAS_total <- rowSums(df[,c( "work", "home", "social", "private", "relationships")], na.rm = T)             
...
# Save to file
write.csv(df, "/Users/hollabharath/Documents/Projects/ADBS/WSAS.csv")
